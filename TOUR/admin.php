<?php
	include "assets/controllers/check_admin.php";
	include "connect.php";
	...
	while($row = $result->fetch_assoc()) {
		$adv = ($row["status"] == "Новый") ? '
			<form action="assets/controllers/confirm_order.php" class="w100" method="POST">
				<input type="hidden" value="'.$row["order_id"].'" name="id" />
				<button>Подтвердить завку</button>
			</form>
			<h3 class="text-center">Отменить заявку</h3>
			<form action="assets/controllers/cancel_order.php" class="w100" method="POST">
				<input type="hidden" value="'.$row["order_id"].'" name="id" />
...
?>
	...
	<main>
		...
		<div class="content">
			...
			<div class="head">Добавить отель.</div>
			<form enctype="multipart/form-data" action="assets/controllers/add_product.php" method="POST">
